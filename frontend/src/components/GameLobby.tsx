...
export const GameLobby: React.FC = () => {
    ...
    useEffect(() => {
        ...
        newConnection.on('LobbyCreated', (lobbyId: string, hostName: string) => {
            setLobbyId(lobbyId);
            setInLobby(true);
            setIsHost(true);
        ...
        newConnection.on('PlayerJoined', (playerName: string, isHost: boolean) => {
            setPlayers(prev => [...prev, { name: playerName, isHost: isHost }]);
            setError('');
        });

        newConnection.on('PlayerLeft', (playerName: string) => {
            setPlayers(prev => prev.filter(p => p.name !== playerName));
        });

        newConnection.on('NewHost', (playerName: string) => {
